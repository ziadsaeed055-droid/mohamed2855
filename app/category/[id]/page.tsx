...
export default function CategoryPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // If a subcategory is selected, filter by it. Otherwise, filter by main category
        if (selectedSubCategory) {
          ...
          q = query(collection(db, "products"), where("category", "==", id), where("isActive", "==", true))
        }
  ...
  const breadcrumbLabel = language === "ar" ? category.nameAr : category.nameEn
        ...
        <div className="mb-8 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          <div className="flex flex-col gap-2">
            {subCategories.length > 0 && (
              <div>
                <p className="text-gray-600 font-semibold mb-2">{language === "ar" ? "الفئات" : "Categories"}</p>
          ...
          {/* Sort */}
          <div>
            <p className="text-gray-600 font-semibold mb-2">{language === "ar" ? "الترتيب" : "Sort By"}</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">
              {language === "ar" ? "لا توجد منتجات في هذا القسم حالياً" : "No products in this category yet"}
            </p>
